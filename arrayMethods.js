...
const afterTax = prices.map(price => price * 1.07) 
...
// CODE HERE
const totalPopulation = populations.reduce((acc, popNum)=>{
  return acc + popNum
})
...
// .reduce((acc, cpAmount) => acc + cpAmount)
...
const salesTaxAdded = orders.reduce((acc, currVal)=> {
  return acc = (currVal.price + ( currVal.price * currVal.tax)
)})
...
*/
// const bobsTotal = purchase.filter(purch => purch.owner === "Bob").map(purch => purch.price).reduce((a,p) => a + p)
